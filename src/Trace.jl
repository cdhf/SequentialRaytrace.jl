...
function gen_result(::Type{Array{Ray{T},1}}, lens::Lens{T}) where {T}
    [
        Ray(zero(T), zero(T), zero(T), zero(T), zero(T))
        for i in range(1, stop = 2 + n_surfaces(lens))
    ]
end
...
function trace!(result, lens, ray::Ray{T}, wavelength; ignore_apertures = false) where {T}
    ...
    after_surfaces = trace_components!(
        result,
        global_index,
        ray_c,
        n,
...
function trace_components!(
    ...
    if length(components) > 0
        after_surfaces = trace_component!(
            result,
            global_index,
            ray,
            n,
